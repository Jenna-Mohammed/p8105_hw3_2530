...
The plot shows an exponential graph
```{r}
instacart |> 
...
The spaghetti plot above depicts the average data values from 2002 through 2010 among all states in the United States.
...
The above graph depicts the distribution of data value response quality in NY state in 2006 and 2010. The two plots follow a similar distribution, however the 2010 graph shows higher density compared to 2006.
...
The boxplots show that the mean age of males and females with `More than high school education` is lower compared to those with `Less than high school` and `high school equivalent`. For `High school equivalent`, the mean agge of males is lower than the mean age of females.
...
### 3 panel plot that shows activity throughout the course of a dat
```{r}
